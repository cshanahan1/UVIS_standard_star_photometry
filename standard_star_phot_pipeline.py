...
from WFC3_phot_tools.staring_mode import aperture_phot, utils
...
def main_standard_star_phot_pipeline(get_new_data=False, redownload_data=False,
                                     sort_new_data=False, run_ap_phot=True,
                                     pamcorr=True,
        ...
        for dirr in dat_dirs:
            ...
            for i, f in enumerate(files): 
                ...
                if pamcorr:
                    ...
                    dat = make_PAMcorr_image_UVIS(copy.deepcopy(dat),
                                                 hdu[0].header,
                                                 hdr, PAM_PATH)
                ...
                phot_tab = _add_hdr_values_to_phottab(phot_tab, hdr, keywords=['phtratio'])
                phot_tab['back'] = back
                ...
                for rad in CIRCULAR_APERTURE_RADII:
                    ...
                    flux_error = compute_phot_err_daophot(flux,
                                                          back*exptime,
                                                          back_std*exptime, 
                                                          phot_ap_area,
                                                          sky_ap_area, 
                                                          gain=1.0)
                    phot_tab['err_{}'.format(rad)] = flux_error / exptime
                if (i == 0) or (j == 0):
            ...
            print(phot_file_path)
            #phot_tab_all.write(phot_file_path, format='ascii', delimiter=' ', overwrite=True)
